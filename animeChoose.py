...
def openExisting(filePath):
    '''Opens a list from a file, assumed to already be appropriately ordered 
    and formatted.'''
    try:    
...
def findSwapProbability(i,j,m_i, m_j):
    '''Returns the (non-normalised) probability of swapping tasks 
    i and j in given list'''
    E_1 = (i*m_i) + (j*m_j)
    E_2 = (i*m_j) + (j*m_i)
...
### Code from http://stackoverflow.com/questions/3679694/
...
def thermalShuffle(taskArray):
    '''Shuffle the tasks according to thermal-motion based laws. I think a 
    good analogy would be a gas of particles of mixed masses at relatively low 
    temperature, in a fairly vertical container.'''    
    # Loop over the tasks:
    if args.verbose:
